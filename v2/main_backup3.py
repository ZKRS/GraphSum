...
from util import logger, save_model, read_txt_data, read_json_data, preprocess, extract_keyword, \
    cal_word_freq, \
...
class GraphConvolutionNet(torch.nn.Module):
    def __init__(self, hps, embedded):
        ...
        i = 0
        self.layers.append(
            GraphConvolutionLayer(hps.w_input_dim, hps.s_input_dim, hps.w_hidden_dim[i], hps.s_hidden_dim[i]))
        for i in range(self.layer_num - 1):
            # w_input_dim, s_input_dim, w_output_dim, s_output_dim
            self.layers.append(
                GraphConvolutionLayer(hps.w_hidden_dim[i], hps.s_hidden_dim[i], hps.w_hidden_dim[i + 1],
                                      hps.s_hidden_dim[i + 1]))
        self.layers.append(
            GraphConvolutionLayer(hps.w_hidden_dim[i], hps.s_hidden_dim[i], hps.w_output_dim, hps.s_output_dim))
    ...
    def forward(self, g, score, g_cls):
        ...
        sent_pair = torch.sigmoid(torch.mul(weight, torch.bmm(s_h_reshape, s_h_reshape.permute(0, 2, 1))))
...
class FocalLoss(torch.nn.Module):
    ...
    def forward(self, predict, labels):
        ...
        pos = torch.full(labels.shape, 0.1, device=torch.device("cuda"))
        neg = torch.full(labels.shape, 0.9, device=torch.device("cuda"))
...
def validate(hps, model, validate_loader, validate_dataset, best_loss, best_F, non_descent_cnt, saveNo):
    ...
    with torch.no_grad():  # 不需要进行梯度计算
        for i, (G, G_cls, index) in enumerate(validate_loader):
            ...
            G_cls = G_cls.to(torch.device("cuda"))
    ...
    if not best_loss or running_avg_loss < best_loss:  # 第一次或者loss下降的情况
        bestmodel_save_path = os.path.join(eval_dir, "bestmodelbp_%d" % (saveNo % 3))
        if best_loss:  # 不为空
            f_logger.info(
    ...
    if not best_F or best_F < tester.F:
        bestmodel_save_path = os.path.join(eval_dir, "bestbpFmodel")
        if best_F:
...
def train(hps, model, train_loader, validate_loader):
    ...
    criterion = Loss(FocalLoss(hps.loss_alpha, hps.loss_beta), hps.batch_size).cuda()
    # # train
    ...
    for epoch in range(1, hps.epoch + 1):
        ...
        for i, (G, G_cls, index) in enumerate(train_loader):
            try:
                ...
                model.train()
                initiate_scores = torch.tensor(hps.batch_size * [[1 / sent_num] * sent_num]).cuda()
                ...
                if i % 10000 == 0:
                    f_logger.info('       | end of iter {:3d} | time: {:5.2f}s | train loss {:5.6f} | '
                                  .format(i, (time.time() - iter_start_time), float(train_loss / 10000)))
        ...
        if not best_train_loss or epoch_avg_loss < best_train_loss:
            save_file_path = os.path.join(hps.train_dir, "bestmodelbp")
            ...
            save_model(model, os.path.join(hps.train_dir, "earlystopbp"))
            sys.exit(1)
...
def setup_training(hps, model, train_loader, validate_loader):
    ...
    try:
        ...
        save_model(model, os.path.join(train_dir, "earlystopbp"))
...
class Tester:
    def __init__(self, hps):
        ...
        self.hps = hps
        self.criterion = Loss(FocalLoss(hps.loss_alpha, hps.loss_beta), hps.batch_size).cuda()
    ...
    def validate(self, model, G, G_cls, validate_dataset, indexes):
        ...
        labels = G.ndata['label']['sentence'].reshape(self.hps.batch_size * sent_num * sent_num, -1).cuda()
        initiate_scores = torch.tensor(self.hps.batch_size * [[1 / sent_num] * sent_num]).cuda()
        # scores: [batch_size,sent_num]  s_h:[batch_size*sent_num, dim]  super_h:[batch_size,dim]
        # sent_pair:[batch_size*sent_num*sent_num, 2]
...
if __name__ == '__main__':
    class Hps:
        def __init__(self):
            ...
            self.w_output_dim = 64  # 300
            self.s_output_dim = 64  # 200
            self.w_hidden_dim = [128]
            self.s_hidden_dim = [128]
            self.lr = 1e-4
            self.epoch = 20
            self.dropout = 0.1
            self.batch_size = 16
    ...
    # hps.local_rank = local_rank
    # hps.device = torch.device("cuda", local_rank)

    # device = [torch.device(f'cuda:'+i) for i in hps.local_rank]
    ...
    train_dataset = DatasetGraph(load_graphs(hps.val_dataset_graphs_path)[0], hps.validate_data_path)
    # train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset)
    train_loader = DataLoader(train_dataset, batch_size=hps.batch_size,
                              num_workers=hps.num_workers,
                              collate_fn=PadBatchGraph(vocab), drop_last=True, shuffle=hps.shuffle)
    ...
    validate_loader = DataLoader(validate_dataset, batch_size=hps.batch_size,
                                 num_workers=hps.num_workers,
                                 collate_fn=PadBatchGraph(vocab), drop_last=True, shuffle=hps.shuffle)
    ...
    # 模型
    model = GraphConvolutionNet(hps, embeded).cuda()
    if torch.cuda.device_count() > 1:
        model = nn.DataParallel(model, device_ids=hps.local_rank)
    # train
    setup_training(hps, model, train_loader, validate_loader)

# CUDA_VISIBLE_DEVICES=0,3 python -m torch.distributed.launch --nproc_per_node=2 main_backup3.py
