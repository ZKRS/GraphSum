...
class GraphConvolutionNet(torch.nn.Module):
    ...
    def forward(self, g, score, g_cls):
        ...
        f_logger.info("[Info] s_h")
        f_logger.info(s_h)
        f_logger.info("[Info] super_h")
        f_logger.info(super_h)
        # set g_cls embedding
...
def validate(hps, model, validate_loader, validate_dataset, best_loss, best_F, non_descent_cnt, saveNo):
    ...
    eval_dir = os.path.join(hps.model_root, "eval")
...
def train(hps, model, train_loader, validate_loader):
    ...
    model.load_state_dict(torch.load(hps.model_root + '/train/bestmodel'))
    best_loss, best_F, non_descent_cnt, saveNo = validate(hps, model, validate_loader, validate_dataset, best_loss,
                                                          best_F, non_descent_cnt, saveNo)
...
class Tester:
    ...
    def validate(self, model, G, G_cls, validate_dataset, indexes):
        ...
        """
  "      p_cls = torch.ones(sent_pair.shape[0]).cuda(gpu)
        n_cls = torch.zeros(sent_pair.shape[0]).cuda(gpu)
        cls = torch.where(sent_pair[:, 1] > 0.5, p_cls, n_cls).reshape(self.hps.batch_size, sent_num,
                                                                       sent_num).cpu()
                                                                       """
...
if __name__ == '__main__':
    class Hps:
        def __init__(self):
            self.data_root = "/home/wangqing/project/data/GraphSumData"
            self.model_root = "/home/wangqing/project/model/GraphSumModel"
            self.train_data_path = self.data_root + "/cnndm/train.label.jsonl"
            self.validate_data_path = self.data_root + '/cnndm/val.label.jsonl'
            self.test_data_path = self.data_root + '/cnndm/test.label.jsonl'
            self.vocab_path = self.data_root + "/vocab.txt"
            self.filter_word_path = self.data_root + "/filter_word.txt"
            self.word_embedding_path = self.model_root + "/cnndm_word2vec.model"
            self.train_dir = self.model_root + '/train/'
            self.train_dataset_graphs_path = self.data_root + '/graph/train_graph.bin'
            self.val_dataset_graphs_path = self.data_root + '/graph/val_graph.bin'
            self.test_dataset_graphs_path = self.data_root + '/graph/test_graph.bin'
            self.device_ids = [0, 1]
            ...
            self.save_root = self.model_root
