...
class DatasetGraph(Dataset):
    def __init__(self, dataset_graphs):
    ...
    def get_doc_item(self, index, data_path):
        docs = read_json_data(data_path)
        return docs[index]
    ...
    def __create_graph__(self, words, text, label):
        ...
        for i in range(len(sent_matrix_id)):
            for j in range(i + 1, len(sent_matrix_id)):
                ...
                if intersect_word_id != {UNKNOWN_TOKEN_ID} and intersect_word_id != {PAD_TOKEN_ID} and len(intersect_word_id) > 0:
                    ss_edges[0].append(i)
                    ss_edges[1].append(j)
...
if __name__ == "__main__":
    ...
    dataset_graphs = DatasetGraph(vocab_path, filter_word_path, max_keyword_num)
